...
public class UIFFM implements UIAPI {
  ...
  public int[] uiLoadPNG(byte[] data,int length,int[] dim) { try { Arena arena = Arena.ofAuto(); MemorySegment _array_data = FFM.toMemory(arena, data);MemorySegment _array_dim = FFM.toMemory(arena, dim);FFM.createFFMArray();uiLoadPNG.invokeExact(_array_data,length,_array_dim);FFM.copyBack(_array_data,data);FFM.copyBack(_array_dim,dim);return (int[])FFM.getArray(); } catch (Throwable t) { JFLog.log(t);  return null;} }

  private MethodHandle uiSavePNG;
  public byte[] uiSavePNG(int[] pixels,int width,int height) { try { Arena arena = Arena.ofAuto(); MemorySegment _array_pixels = FFM.toMemory(arena, pixels);FFM.createFFMArray();uiSavePNG.invokeExact(_array_pixels,width,height);FFM.copyBack(_array_pixels,pixels);return (byte[])FFM.getArray(); } catch (Throwable t) { JFLog.log(t);  return null;} }

  private MethodHandle uiLoadJPG;
  public int[] uiLoadJPG(byte[] data,int length,int[] dim) { try { Arena arena = Arena.ofAuto(); MemorySegment _array_data = FFM.toMemory(arena, data);MemorySegment _array_dim = FFM.toMemory(arena, dim);FFM.createFFMArray();uiLoadJPG.invokeExact(_array_data,length,_array_dim);FFM.copyBack(_array_data,data);FFM.copyBack(_array_dim,dim);return (int[])FFM.getArray(); } catch (Throwable t) { JFLog.log(t);  return null;} }

  private MethodHandle uiSaveJPG;
  public byte[] uiSaveJPG(int[] pixels,int width,int height,int quality) { try { Arena arena = Arena.ofAuto(); MemorySegment _array_pixels = FFM.toMemory(arena, pixels);FFM.createFFMArray();uiSaveJPG.invokeExact(_array_pixels,width,height,quality);FFM.copyBack(_array_pixels,pixels);return (byte[])FFM.getArray(); } catch (Throwable t) { JFLog.log(t);  return null;} }
  ...
  public long uiWindowCreate(int style,String title,int width,int height,long shared) { try { Arena arena = Arena.ofAuto(); long _ret_value_ = (long)uiWindowCreate.invokeExact(style,arena.allocateFrom(title),width,height,shared);return _ret_value_; } catch (Throwable t) { JFLog.log(t);  return -1;} }
  ...
  public void uiPollEvents(long id,int wait,UIEvents events) { try { FFM.setUIEvents(events);uiPollEvents.invokeExact(id,wait,FFM.upcall_UIEvents_dispatchEvent); } catch (Throwable t) { JFLog.log(t); } }
